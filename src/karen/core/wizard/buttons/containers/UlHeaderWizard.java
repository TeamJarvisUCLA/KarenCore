...
public class UlHeaderWizard extends Div {
	...
	private final String urlUl = "assets/theme/components/ulHeaderWizard.zul";
	...
	public void createHeader(List<String> iconos) {
		...
		for (int i = 0; i < iconos.size(); i++) {
			...
			LiHeaderWizard liHeaderWizard = new LiHeaderWizard(liStyle,
					(i == 0) ? "active" : "disabled", "", icono);
		...
		Div divInner = (Div) Executions.createComponents(urlUl, null,
				parametros);
	...
	public void nextIcon() {
		Clients.evalJavaScript("jq('.wizard .nav-tabs li.active').removeClass('active').addClass('disabled').next().removeClass('disabled').addClass('active')");
	}

	public void previousIcon() {
		Clients.evalJavaScript("jq('.wizard .nav-tabs li.active').removeClass('active').addClass('disabled').prev().removeClass('disabled').addClass('active')");
	}
}
