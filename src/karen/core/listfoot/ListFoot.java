...
public class ListFoot extends Listfooter {
	...
	private ListFootButtonClickListener listenerButtonClickListener;
	...
	private void addComponents() {
		...
		if (howToSeeCurrent.equals(HowToSeeEnum.NORMAL)) {
			...
			btnHowSeeData.setIconSclass("fa fa-sort-amount-asc");
			btnHowSeeData
					.setTooltiptext("Cambiar al modo 'Paginación Extendida'");
		} else {// EXTENDED
			divButtons.getChildren().add(btnNextPage);
	...
	private void addEvents() {
		...
		btnRefresh.addEventListener(Events.ON_CLICK,
				new EventListener<Event>() {
		...
		btnHowSeeData.addEventListener(Events.ON_CLICK,
				new EventListener<Event>() {
	...
	private void lanzarEvento() {
		if (listenerButtonClickListener != null) {
			listenerButtonClickListener
					.onClickPaginacion(new ListFootButtonClickEvent(page));
		} else {
		...
		try {
			...
			if (howToSeeCurrent.equals(HowToSeeEnum.NORMAL)) {
				if (totalElementsReturned == 0) {
					...
					lblRegisters
							.setValue("La consulta no ha devuelto registros");
					return;
				} else {
				...
				if (page > 0) {
					lblRegisters
							.setValue((page * totalElementsByPage
									- totalElementsByPage + 1)
									+ " - "
									+ (page * totalElementsByPage
	...
	public ListFootButtonClickListener getListenerButtonClickListener() {
		return listenerButtonClickListener;
	}

	public void setListenerButtonClickListener(
			ListFootButtonClickListener listenerButtonClickListener) {
		this.listenerButtonClickListener = listenerButtonClickListener;
	}
