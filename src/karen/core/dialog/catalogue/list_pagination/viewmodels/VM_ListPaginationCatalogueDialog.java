...
public abstract class VM_ListPaginationCatalogueDialog<T> extends
		VM_SimpleListCatalogueDialog<T> {
	...
	public IPayloadResponse<T> updateListBox(Integer page,
		...
		IPayloadResponse<T> iPayload = getObjectListToLoad(
				C_ListPaginationCatalogueDialog.CANTIDAD_REGISTROS_PAGINA_DEFECTO,
				Math.abs(page));
	...
	public C_ListPaginationCatalogueDialog<T> getControllerListPaginationCatalogueDialog() {
		return (C_ListPaginationCatalogueDialog<T>) getControllerWindowDialog();
	}

	public abstract IPayloadResponse<T> getObjectListToLoad(
